...
class AlarmClockService extends Service {
   ...
   *
   * @param {Object} [options] - An object with the following properties
   * @param {number} options.ID - The Alarm ID, see ListAndParseAlarms
   */
  ...
  /**
   * ListAlarms - Get the AlarmList as XML, use ListAndParseAlarms for parsed version
  ...
  /**
   * UpdateAlarm - Update an alarm, all parameters are required. Use PatchAlarm where you can update a single parameter
   *
   * @param {Object} [options] - An object with the following properties
   * @param {number} options.ID - The ID of the alarm see ListAndParseAlarms
   * @param {string} options.StartLocalTime - The start time as hh:mm:ss
   * @param {string} options.Duration - The duration as hh:mm:ss
   * @param {string} options.Recurrence - Repeat this alarm on [ 'ONCE' / 'WEEKDAYS' / 'WEEKENDS' / 'DAILY' ]
   * @param {boolean} options.Enabled - Alarm enabled after creation
